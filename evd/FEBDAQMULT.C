...
TGMainFrame *fMainFrame1314 = nullptr;
...
{
  // Save data tree
  Count_fTime=Count_fTime+1;

  std::string treePath = "output/mppc.root" ;
  std::string anaPath  = "output/ana_"+std::to_string(Count_fTime)+".root";
  std::string dataPath = "output/data.txt";
  tr->SaveAs(treePath.c_str());


  // Trigger the analysis script
  std::string doAnaCmd = "doAnalyze(\""+treePath+"\", \""+anaPath+"\",\""+dataPath+"\")";
  cout << doAnaCmd << endl;
...
void StartDAQForEvd(int nev=0)
{
  ...
  // Create a timer to save the data tree every so often
  if (!fTimer) 
  {
    fTimer = new TTimer();
    fTimer->Connect("Timeout()", 0, 0,"HandleTimer()");
    fTimer->Start(fTime, kFALSE);
...
void FEBGUI()
{
   ...
   // main frame
   fMainFrame1314 = new TGMainFrame(gClient->GetRoot(),10,10,kMainFrame | kVerticalFrame);
   ...
   TGTextButton *fTextButton882 = new TGTextButton(fGroupFrame679,"Start DAQ Event Display");
   fTextButton882->SetTextJustify(36);
   fTextButton882->SetMargins(0,0,0,0);
   fTextButton882->SetWrapLength(-1);
   fTextButton882->Resize(123,22);
   fGroupFrame679->AddFrame(fTextButton882, new TGLayoutHints(kLHintsLeft| kLHintsCenterX  | kLHintsTop | kLHintsExpandX,0,0,2,2));
   fTextButton882->SetCommand("if(RunOn==0) StartDAQForEvd();");
    
   TGTextButton *fTextButton883 = new TGTextButton(fGroupFrame679,"Stop DAQ Event Display");
   fTextButton883->SetTextJustify(36);
   fTextButton883->SetMargins(0,0,0,0);
   fTextButton883->SetWrapLength(-1);
   fTextButton883->Resize(123,22);
   fGroupFrame679->AddFrame(fTextButton883, new TGLayoutHints(kLHintsLeft| kLHintsCenterX  | kLHintsTop | kLHintsExpandX,0,0,2,2));
   fTextButton883->SetCommand("RunOn=0; StopDAQForEvd();");
